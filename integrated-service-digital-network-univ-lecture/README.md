...
- 물리 주소와 논리 주소
  ...
  - 물리 주소
    ...
    - 48비트로 구성
    - 콜론으로 8비트씩 구분하여 6개의 16진수로 표현

  - 논리 주소

    - 네트워크 계층에서 사용하는 IP주소
    - 32비트로 구성
    - 점으로 8비트씩 구분하여 4개의 10진수로 표현

### 오류제어

- 오류

  - 송신 데이터와 수신 데이터가 다른 경우
...
- 오류 제어
  ...
  - 전진 오류 정정
    ...
    - 연속적인 데이터 흐름이 필요하고 역방향 채널의 제공이 어려울 때 사용

    - 오류 정정 코드를 전송

      - 데이터 프레임에 잉여 비트를 추가하여 반듦

      - 해밍(Hamming) 코드

      - 잉여 비트의 크기만큼 전송 효율이 떨어짐

  - 귀환 오류 제어

    - 결정 귀환

      - 오류 검출 위치사 수신측에 있는 경우
        ...
        - 연속적 ARQ
          ...
          - 여러 개의 데이터 프레임을 송신하고 기다림
            ...
            - Go-Back-N QRQ
              ...
              - sliding window protocol 사용

              - ACK 수신하면 다음 window만큼 송신

              - NAK(n)을 수신하면 n 부터 window 사이즈만큼 송신

      - 적응적 ARQ

    - 정보 귀환

      - 오류 검출의 위치가 숭신측에 있는 경우
...
- 장단점
  ...
  - 단점 : UDP를 사용하는 응용 프로그램은 메시지 손실, 중복 수신, 수신 지연, 잘못된 순서 등을 처리하기 위한 신뢰성 제어 기능을 제공해야 함
...
- TCP 정의
  ...
  - 신뢰성
    ...
    - 흐름제어 이용 (sliding window protocol)
