...
클래스나 함수를 특정 자료형에 종속되지 않도록, 객체의 자료형을 인수로 처리하여 일반적은 자료형을 대상으로 선언하는 방법.

## 표준 템플릿 라이브러리 (STL)

C++ 에서 제공되는 표준 컨테이너 클래스 템플릿 라이브러리. 벡터, 리스트, 스택, 큐 등의 컨테이너와 이들을 처리하기 위해 사용할 수 있는 여러가지 연산을 포함한다.

### 구성요소

- 컨테이너(contatiner) : 데이터 저장
- 반복자(iterator) : 포인터 역할
- 알고리즘 (algorithm) : 데이터 처리 기능

#### 순차 컨테이너

- vector : 크기 확장이 가능한 배열
- list : 이중 연결 리스트
- deque : vector와 list의 혼합 형태

#### 연상 컨테이너

- set : 키 객체만 저장하며, 키가 중복되지 않음
- multiset : 키 객체만 저장하며, 동일한 키가 중복될 수 있음
- map : 키, 값 의 쌍을 저장. 키가 중복되지 않음
- multimap : 키, 값 의 쌍을 저장. 키가 중복될 수 있음

#### 무순서 연상 컨테이너

- unorderd_set : 키 객체만 저장하며, 키가 중복되지 않음
- unorderd_multiset : 키 객체만 저장하며, 동일한 키가 중복될 수 있음
- unorderd_map : 키, 값 의 쌍을 저장. 키가 중복되지 않음
- unorderd_multimap : 키, 값 의 쌍을 저장. 키가 중복될 수 있음

set, multiset, map , multimap 과 같으나 해시함수를 이용하여 검색 함

#### 컨테이너 어댑터

- queue : FIFO 구조
- priority_queue : 우선 순위에 따라 데이터를 엑세스할 수 있는 구조
- stack : LIFO 구조
...
- 순방향(forward) 반복자 : 컨테이너의 순방향으로만 움직임. ++ 연산자 사용
- 양방향(bidirectional) 반복자 : 컨테이너의 순방향과 역방향으로 움직일 수 있음. ++, -- 연산자 사용
- 랜덤 엑세스 반복자 : 양방향 반복자의 기능과 함께 임의의 위치로 이동할 수 있음

### 알고리즘

- search : 지정된 값과 동일한 첫번째 원소를 반환
...
- [예제 소스 보기](./CppPrj/Vector1/Vector1.cpp)
